...
CREATE TABLE IF NOT EXISTS item (
    ...
    median_price_rm NUMERIC(12, 2),
    CONSTRAINT item_median_price_positive
...
-- English labels are maintained on the canonical PriceCatcher lookup row.
-- NULL is intentional when the source item name is blank or has not yet been
-- translated; it must never be interpreted as an available price or stock.
ALTER TABLE item ADD COLUMN IF NOT EXISTS item_name_en VARCHAR(500);
ALTER TABLE item ADD COLUMN IF NOT EXISTS median_price_rm NUMERIC(12, 2);
...
CREATE TABLE IF NOT EXISTS category_translation (
    ...
    CONSTRAINT category_translation_locale_check
...
COMMENT ON COLUMN item.item_name_en IS
    'Code-keyed English label from the maintained lookup seed; NULL when the official source name is blank.';
...
CREATE INDEX IF NOT EXISTS category_translation_name_idx
    ON category_translation (translated_name);
